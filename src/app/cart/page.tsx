...
export default function CartPage() {
    ...
    // Populate billing address with user data when logged in
    useEffect(() => {
        if (user) {
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        ...
                        <div className="lg:col-span-1">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border sticky top-28">
                                {!loading && (
                                    user ? (
                                        // Logged-in user: Show simplified billing form
                                        <>
                                        ...
                                        // Non-logged user: Show login/signup options
                                        <>
                                            <h2 className="text-lg font-semibold font-headline text-gray-800">Login or Create Account</h2>
                                            <p className="text-sm text-gray-600 mt-2 mb-4">Please login or create an account to continue with checkout.</p>
                                            <CartAuthForm total={totalUSD} />
                                            <Separator className="my-6" />
                                        </>
                                    )
                                )}
                                ...
                                <Button 
                                    size="lg" 
                                    className="w-full mt-4 bg-black text-white hover:bg-gray-800 text-base" 
                                    onClick={handleCheckout} 
                                    disabled={isProcessing || totalUSD <= 0 || !user || (user && (!billingAddress.name || !billingAddress.mobile || !billingAddress.address))}
                                >
                                    {isProcessing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                    {isProcessing ? 'Processing...' : !user ? 'Please login to continue' : 'Continue to checkout'}
                                </Button>
                            </div>
                        </div>
                    </div>
