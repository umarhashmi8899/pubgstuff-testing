...
export default function AdminBannerPage() {
  ...
  const handleAddBanner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBanners((prevBanners) => [data, ...prevBanners])
      setNewBannerText("")
  ...
  const handleToggleActive = async (id: number) => {
    ...
    const { error } = await supabase.from("banners").update({ active: newStatus }).eq("id", id)
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <form onSubmit={handleAddBanner} className="flex flex-col sm:flex-row items-end gap-4">
            ...
            <div className="w-full sm:w-48">
              ...
              <Select
                onValueChange={(value: "info" | "discount" | "warning") => setNewBannerType(value)}
                defaultValue={newBannerType}
              >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage your existing banners here. You can activate or deactivate them. Multiple active banners will rotate
            on the website.
          </CardDescription>
        </CardHeader>
        <CardContent>
