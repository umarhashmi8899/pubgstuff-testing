...
const secondaryNav = [
    { href: "/admin/reports", label: "Reports", icon: <MessageSquareWarning className="h-6 w-6" />, notificationCount: 1 },
...
export default function AdminSidebar() {
    ...
    return (
        <Sidebar collapsible="icon" className={cn(isMobile ? "flex" : "hidden sm:flex", "bg-card text-card-foreground border-r")}>
            ...
            <SidebarContent className="flex flex-col">
                <SidebarMenu className="flex-1 gap-2 pt-2">
                    ...
                    {secondaryNav.map((link) => {
                        const count = link.label === 'Reports' ? newReportsCount : link.label === 'Suggestions' ? newSuggestionsCount : 0;
                        return (
                            <SidebarMenuItem key={link.href}>
                                <SidebarMenuButton
                                    asChild
