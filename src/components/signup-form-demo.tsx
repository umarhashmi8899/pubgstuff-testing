...
export default function SignupFormDemo() {
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (passwordStrength < 5) {
      ...
      setError(msg);
      toast({ title: "Weak Password", description: "Please choose a stronger password that meets all requirements.", variant: "destructive" });
      return;
    }
  ...
  const handleSocialSignIn = async (provider: "google" | "facebook") => {
    // Social sign-in functionality removed - implement with your preferred OAuth provider
    toast({
      title: "Feature Unavailable",
      description: "Social sign-in is currently unavailable.",
      variant: "destructive",
    });
  };
  ...
  return (
    <div className="shadow-input w-full rounded-none bg-white p-4 md:rounded-2xl md:p-8 dark:bg-black">
      ...
      <form className="my-8" onSubmit={handleSignup}>
        ...
        <button
          className="group/btn hover-shimmer-button relative flex items-center justify-center h-10 w-full rounded-md bg-gradient-to-br from-black to-neutral-600 font-medium text-white shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] dark:bg-zinc-800 dark:from-zinc-900 dark:to-zinc-900 dark:shadow-[0px_1px_0px_0px_#27272a_inset,0px_-1px_0px_0px_#27272a_inset]"
          type="submit"
          disabled={loading}
        >
